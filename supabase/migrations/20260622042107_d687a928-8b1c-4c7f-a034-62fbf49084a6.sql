...
DECLARE
  ...
  v_caixa public.caixas_pdv%ROWTYPE;
  v_order_id uuid;
  v_order_number text;
...
BEGIN
  ...
  INSERT INTO public.orders (
    ...
    v_op.organization_id, 'preparing', _items, COALESCE(_total,0), 'pdv', COALESCE(_forma,'dinheiro'),
    'Balcão (' || v_op.name || ')', '',
    CASE WHEN COALESCE(_cupom_code,'') <> '' THEN 'Cupom: '||_cupom_code ELSE '' END,
    v_order_number
