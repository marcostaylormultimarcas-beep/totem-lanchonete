-- VisionFood V2 PHASE 276
-- validate_checkout_coupon already rejects a coupon when either configured
-- expiry boundary (data_fim or legacy validade) has elapsed.
-- Align the authoritative checkout quote with that same contract.
--
-- Confirmed divergence before this migration:
-- - validate_checkout_coupon checks data_fim and validade independently;
-- - pdv_validar_cupom_v2 checks data_fim and validade independently;
-- - quote_order_checkout used COALESCE(data_fim, validade), so a future
--   data_fim could mask an already-expired legacy validade.
--
-- This migration intentionally does not change EXECUTE grants.
...
begin
 ...
 if nullif(btrim(coalesce(_coupon_code,'')),'') is not null then
   ...
   if coalesce(_coupon.data_inicio,'-infinity'::timestamptz)>now()
      or (_coupon.data_fim is not null and _coupon.data_fim<now())
      or (_coupon.validade is not null and _coupon.validade<now())
   then
