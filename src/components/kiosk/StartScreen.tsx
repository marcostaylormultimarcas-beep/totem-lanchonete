...
const StartScreen = ({ onStart, onAddToCart }: StartScreenProps) => {
  ...
  // Top products (first 4 from hamburgueres + pizzas)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
            ...
            {banners.map((banner, i) => (
              ...
              >
                <div className="flex-1 space-y-2 z-10">
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${badgeColorMap[banner.badgeColor] || badgeColorMap.primary}`}>
                    {banner.badgeText}
                  </span>
                ...
                {banners.map((_, i) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all duration-300 ${
                      i === activeBanner ? 'bg-white w-5' : 'bg-white/40'
                    }`}
                  />
                ))}
              </div>
          ...
          {topProducts.map(product => (
            <div key={product.id} className="kiosk-card group relative">
              <button
                ...
                className="w-full flex flex-col items-center p-3 sm:p-4 gap-2 text-center"
              >
                {isUrl(product.image) ? (
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-background via-background to-transparent z-30">
        <button
          onClick={onStart}
          className="touch-btn w-full bg-primary text-primary-foreground py-5 text-xl font-black rounded-2xl pulse-glow"
        >
          FAZER PEDIDO
        </button>
      </div>
