-- PHASE 304 follow-up: secure manual table selection for enrolled physical kiosks.
-- QR public_token remains private to the QR flow. The companion receives only private table ids
-- after authenticating the enrolled device and can cache the list for offline use.

create or replace function public.visionfood_kiosk_tables(
  _device_id uuid,
  _credential text
)
returns jsonb
...
begin
  ...
  if coalesce(
    (public.visionfood_public_organization(d.organization_id,null)->>'paused')::boolean,
    true
  ) then
...
revoke all on function public.visionfood_kiosk_tables(uuid,text) from public, authenticated;
grant execute on function public.visionfood_kiosk_tables(uuid,text) to anon, service_role;

-- Allow the device-owned sync contract to resolve a table selected by the enrolled kiosk
-- using the private table UUID, while preserving the existing QR public_token path.
